...
#define MCP4725_COMMON_I2CADDR                                                 \
  ((0x62)                                                                      \
   << 1) // This is set before the main loop by asserting all A0 pins to 1
#define MCP4725_ACTIVE_I2CADDR                                                 \
  ((0x63) << 1) // This is set by asserting 0 to the target DAC's A0 pin prior
                // to write
...
static void MX_I2C1_Init(void) {
   ...
   */
  if (HAL_I2CEx_ConfigAnalogFilter(&hi2c_dac, I2C_ANALOGFILTER_ENABLE) !=
      HAL_OK) {
    Error_Handler();
  }
...
static void MX_I2C2_Init(void) {
  ...
  /** Configure Analogue filter
   */
  if (HAL_I2CEx_ConfigAnalogFilter(&hi2c_lcd, I2C_ANALOGFILTER_ENABLE) !=
      HAL_OK) {
    Error_Handler();
  }
...
HAL_StatusTypeDef I2cSendtoDAC(const GPIO_TypeDef *port, const uint16_t pin,
                               uint8_t *pData, const uint32_t timeout) {
  ...
  // TODO: Check MCP's busy fleg before sending anything

  result = HAL_I2C_Master_Transmit(&hi2c_dac, MCP4725_ACTIVE_I2CADDR, pData, 3,
                                   timeout);

  // Just burn some processor cycles
  asm volatile("nop\n\t"
               "nop\n\t"
               "nop");
  ...
  /* Transmit is defined as blocking,
     ...
     And since we dpon't expect an anwer what should we wait for?
  */
  /*HAL_Delay(10);*/
