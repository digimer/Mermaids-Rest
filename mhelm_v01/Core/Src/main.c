...
int main(void) {
  ...
  // Sleep briefly so the user sees all lights work before turning things off.
  HAL_Delay(1000);
...
static void MX_ADC_Init(void) {
   ...
   */
  // Throttle Starboard Pot, channel number from reference manual for this pin
  ...
  /** Configure for the selected ADC regular channel to be converted.
   */
  // Regen Pot, , channel number from reference manual for this pin
   ...
   */
  // Throttle Port Pot, channel number from reference manual for this pin
...
static void MX_GPIO_Init(void) {
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOA, LD2_Pin | FOOT_SW_OUT_Pin | SPEED_HIGH_SW_OUT_Pin |
                        SPEED_LOW_SW_OUT_Pin, GPIO_PIN_RESET);
  ...
  GPIO_InitStruct.Pin  = MAIN_SW_IN_Pin | THROTTLE_STOP_SW_IN_Pin | SPEED_HIGH_SW_IN_Pin |
                         SPEED_LOW_SW_IN_Pin;
