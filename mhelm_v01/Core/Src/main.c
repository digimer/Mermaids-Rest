...
int main(void) {
  ...
  // TODO: Make a call to write the throttle DAC's EEPROM to their ~2.5v
  // position and the regen DACs to 0.05v

  // uint8_t count = 0;
  uint8_t mainSwitchValue = 0; // This turns the helm displays on (LEDs, etc,
                               // does NOT disable throttle controls)
  uint8_t speedHighSwitchValue =
      0; // 0 = Off, 1 = Selected (inverted at GPIO read)
  uint8_t speedLowSwitchValue =
      0; // 0 = Off, 1 = Selected (inverted at GPIO read)
  ...
  while (1) {
    ...
    throttleStopSwitchValue =
        1 - HAL_GPIO_ReadPin(THROTTLE_STOP_SW_IN_GPIO_Port,
                             THROTTLE_STOP_SW_IN_Pin);
    ...
    if (throttleStopSwitchValue == 1) {
      // Shut everything down! Press the foot switch, change the speed to Low,
      // put the throttle to neutral and Max regen
      speedHighSwitchValue = 0;
      speedLowSwitchValue = 1;
    } else {
      // Do normal logic
      // Read in the speed switches
      speedHighSwitchValue = 1 - HAL_GPIO_ReadPin(SPEED_HIGH_SW_IN_GPIO_Port,
                                                  SPEED_HIGH_SW_IN_Pin);
      speedLowSwitchValue =
          1 - HAL_GPIO_ReadPin(SPEED_LOW_SW_IN_GPIO_Port, SPEED_LOW_SW_IN_Pin);
    }

    // Change output switch values
    HAL_GPIO_WritePin(FOOT_SW_OUT_GPIO_Port, FOOT_SW_OUT_Pin,
                      throttleStopSwitchValue);
    HAL_GPIO_WritePin(SPEED_HIGH_SW_OUT_GPIO_Port, SPEED_HIGH_SW_OUT_Pin,
                      speedHighSwitchValue);
    HAL_GPIO_WritePin(SPEED_LOW_SW_OUT_GPIO_Port, SPEED_LOW_SW_OUT_Pin,
                      speedLowSwitchValue);
    ...
    lcdSetGlobalSwitches(mainSwitchValue, throttleStopSwitchValue,
                         speedHighSwitchValue, speedLowSwitchValue);
 ...
 * This take a value, and two ranges; fromLow, fromHigh and toLow, toHigh. The
 * values relative position between the 'from' values is used to find the
 * corresponding relative function in the 'to' range.
...
static void MX_ADC_Init(void) {
   ...
   */
  sConfig.Channel = ADC_CHANNEL_0; // Throttle Starboard Pot, channel number
                                   // from reference manual for this pin
  ...
  /** Configure for the selected ADC regular channel to be converted.
   */
  sConfig.Channel = ADC_CHANNEL_1; // Regen Pot, , channel number from reference
                                   // manual for this pin
  if (HAL_ADC_ConfigChannel(&hadc, &sConfig) != HAL_OK) {
    Error_Handler();
  }
   ...
   */
  sConfig.Channel = ADC_CHANNEL_13; // Throttle Port Pot, channel number from
                                    // reference manual for this pin
  if (HAL_ADC_ConfigChannel(&hadc, &sConfig) != HAL_OK) {
    Error_Handler();
  }
...
static void MX_GPIO_Init(void) {
  ...
  HAL_GPIO_WritePin(GPIOA,
                    LD2_Pin | FOOT_SW_OUT_Pin | SPEED_HIGH_SW_OUT_Pin |
                        SPEED_LOW_SW_OUT_Pin,
                    GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : DEBUG_0_Pin DEBUG_1_Pin M10KW_THROTTLE_DAC_A0_Pin
   * M10KW_REGEN_DAC_A0_Pin*/
  GPIO_InitStruct.Pin = DEBUG_0_Pin | DEBUG_1_Pin | M10KW_THROTTLE_DAC_A0_Pin |
                        M10KW_REGEN_DAC_A0_Pin;
  ...
  /*Configure GPIO pins : LD2_Pin FOOT_SW_OUT_Pin SPEED_HIGH_SW_OUT_Pin
   * SPEED_LOW_SW_OUT_Pin */
  GPIO_InitStruct.Pin =
      LD2_Pin | FOOT_SW_OUT_Pin | SPEED_HIGH_SW_OUT_Pin | SPEED_LOW_SW_OUT_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);

  /*Configure GPIO pins : MAIN_SW_IN_Pin | THROTTLE_STOP_SW_IN_Pin
   * SPEED_HIGH_SW_IN_Pin SPEED_LOW_SW_IN_Pin */
  GPIO_InitStruct.Pin = MAIN_SW_IN_Pin | THROTTLE_STOP_SW_IN_Pin |
                        SPEED_HIGH_SW_IN_Pin | SPEED_LOW_SW_IN_Pin;
  ...
  GPIO_InitStruct.Pin =
      GPIO_PIN_0 | GPIO_PIN_4 | GPIO_PIN_12 | GPIO_PIN_15 | GPIO_PIN_3;
  ;
  GPIO_InitStruct.Mode = GPIO_MODE_ANALOG;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);

  /*Configure GPIO pins : Enable the LCD1 Prev/Next and throttle select switch
   * pins */
  GPIO_InitStruct.Pin = LCD1_SELECT_PREV_Pin | LCD1_SELECT_NEXT_Pin |
                        MOTOR_5KW_SELECT_Pin | THROTTLE_PORT_SELECT_Pin |
