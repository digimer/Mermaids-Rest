...
void handleMotor(uint16_t throttlePerMille, uint16_t regenPerMille) {
    ...
    if(newMotor != motorActive) {
        ...
        // Think about what happens if we switch back during ramping....
    ...
    // It's a little overkill for only 2 motors, but if we can have more than 2, who knows.
    // Maybe the mermaid will once live on a superyard ;-)
    for(i = 0; i<NUM_MOTORS; ++i ) {
        if(i == motorActive) {
