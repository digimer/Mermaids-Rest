...
const uint16_t MOTOR_MAX_REVERSE[NUM_MOTORS] = {
    THROTTLE_DAC_M5KW_REVERSE_FASTEST, THROTTLE_DAC_M10KW_REVERSE_FASTEST};
const uint16_t MOTOR_MIN_REVERSE[NUM_MOTORS] = {
    THROTTLE_DAC_M5KW_REVERSE_SLOWEST, THROTTLE_DAC_M10KW_REVERSE_SLOWEST};

const uint16_t MOTOR_NEUTRAL[NUM_MOTORS] = {THROTTLE_DAC_M5KW_NEUTRAL,
                                            THROTTLE_DAC_M10KW_NEUTRAL};

const uint16_t MOTOR_MIN_FORWARD[NUM_MOTORS] = {
    THROTTLE_DAC_M5KW_FORWARD_SLOWEST, THROTTLE_DAC_M10KW_FORWARD_SLOWEST};
const uint16_t MOTOR_MAX_FORWARD[NUM_MOTORS] = {
    THROTTLE_DAC_M5KW_FORWARD_FASTEST, THROTTLE_DAC_M10KW_FORWARD_FASTEST};

const uint16_t MOTOR_MIN_REGEN[NUM_MOTORS] = {REGEN_DAC_M5KW_MINIMUM,
                                              REGEN_DAC_M10KW_MINIMUM};
const uint16_t MOTOR_MAX_REGEN[NUM_MOTORS] = {REGEN_DAC_M5KW_MAXIMUM,
                                              REGEN_DAC_M10KW_MAXIMUM};

const GPIO_TypeDef *MOTOR_THROTTLE_DAC_PORTS[NUM_MOTORS] = {
    M5KW_THROTTLE_DAC_A0_GPIO_Port, M10KW_THROTTLE_DAC_A0_GPIO_Port};
const uint16_t MOTOR_THROTTLE_DAC_PINS[NUM_MOTORS] = {
    M5KW_THROTTLE_DAC_A0_Pin, M10KW_THROTTLE_DAC_A0_Pin};

const GPIO_TypeDef *MOTOR_REGEN_DAC_PORTS[NUM_MOTORS] = {
    M5KW_REGEN_DAC_A0_GPIO_Port, M10KW_REGEN_DAC_A0_GPIO_Port};
const uint16_t MOTOR_REGEN_DAC_PINS[NUM_MOTORS] = {M5KW_REGEN_DAC_A0_Pin,
...
void initMotor() {
  ...
  I2C_DEACTIVATE_DAC(M10KW_THROTTLE_DAC_A0_GPIO_Port,
                     M10KW_THROTTLE_DAC_A0_Pin);
  ...
  for (i = 0; i < NUM_MOTORS; ++i) {
    ...
    I2cSendtoDAC(MOTOR_THROTTLE_DAC_PORTS[i], MOTOR_THROTTLE_DAC_PINS[i],
                 throttle_data, 1000);
    I2cSendtoDAC(MOTOR_REGEN_DAC_PORTS[i], MOTOR_REGEN_DAC_PINS[i], regen_data,
                 1000);
  }
}
...
void handleMotor(uint16_t throttlePerMille, uint16_t regenPerMille) {
  ...
  motorSwitch =
      HAL_GPIO_ReadPin(MOTOR_5KW_SELECT_GPIO_Port, MOTOR_5KW_SELECT_Pin);
  // Store it for output
  ...
  if (throttlePerMille == 500) {
    ...
    motorThrottleValue =
        MAP_TO_VALUE(throttlePerMille, MOTOR_MAX_REVERSE[motorActive],
                     MOTOR_MAX_FORWARD[motorActive]);
  }
  motorRegenValue = MAP_TO_VALUE(regenPerMille, MOTOR_MIN_REGEN[motorActive],
                                 MOTOR_MAX_REGEN[motorActive]);
