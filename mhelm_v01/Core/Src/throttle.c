...
// the first half Since we use factor 1000 here we ned 32 bit values for
// calculation The result fits into an uint16 again
uint16_t calcPerMille(uint32_t value, uint32_t low, uint32_t high) {
  return (uint16_t)((value - low) * 1000 / (high - low));
}

// This function reads the necessary values and calculates a throttle position
// in per mille where 0 is reverse fastest, 500 is neutral and 1000 is forward
// fastest Therefore, 0-499 is reverse and 501-1000 is forward We use per mille
// to get a better resolution
uint16_t handleThrottle(uint8_t stopSwitchEngaged) {
  // NOTE: Eventually, if neither are selected, "remote control" will be
  ...
  } else if ((throttleValue > MIN_POINTS_REVERSE[throttleActive]) &&
             (throttleValue < MIN_POINTS_FORWARD[throttleActive])) {
