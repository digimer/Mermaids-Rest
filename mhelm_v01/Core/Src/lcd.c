...
uint8_t lcd1ShowPage =
    3; // Integer changes the data shown on LCD1 (for now, serial)

void lcdSetGlobalSwitches(uint8_t main, uint8_t throttleStop, uint8_t speedHigh,
                          uint8_t speedLow) {
...
void lcdHandlePageButtons() {
  ...
  // required.
  if (HAL_GPIO_ReadPin(LCD1_SELECT_NEXT_GPIO_Port, LCD1_SELECT_NEXT_Pin) ==
      GPIO_PIN_RESET) {
    // NEXT switch pressed
    lcd1ShowPage = (lcd1ShowPage + 1) % lcd1Pages;
  } else if (HAL_GPIO_ReadPin(LCD1_SELECT_PREV_GPIO_Port,
                              LCD1_SELECT_PREV_Pin) == GPIO_PIN_RESET) {
    // PREV switch pressed
    if (lcd1ShowPage == 0) {
...
#define MOTOR_SELECTED status.motorSelect ? 10 : 5
  ...
  case 0:
    ...
    sprintf(buffer,
            "%d - Switches: Main: [%d], E-Stop: [%d], Speed High/Low: [%d/%d], "
            "Throttle Stbd/Port: [%d/%d], Select 5kw Motor: [%d]\n\r",
            counter, status.mainSwitch, status.throttleStopSwitch,
            status.speedHighSwitch, status.speedLowSwitch,
            status.throttleStarboardSelect, status.throttlePortSelect,
            status.motorSelect);
  ...
  case 2:
    ...
    if (status.throttleStopSwitch == 1) {
      sprintf(buffer,
              "%d - Emergency Stop! Speed set to neutral! Raw throttle "
              "position on selected throttle: [%d]\n\r",
              counter, status.throttleValueRaw);
    } else {
      if (status.throttlePortSelect) {
        sprintf(buffer,
                "%d - Port (selected) throttle (raw/per mille/smooth): "
                "[%d/%d/%d], out: [%d] to: [%dkw] motor.\n\r",
                counter, status.throttleValueRaw, status.throttleValuePerMille,
                status.throttleValuePerMilleSmooth, status.throttleValueDac,
                MOTOR_SELECTED);
      } else if (status.throttleStarboardSelect) {
        sprintf(buffer,
                "%d - Starboad (selected) throttle (raw/per mille/smooth): "
                "[%d/%d/%d], out: [%d] to: [%dkw] motor.\n\r",
                counter, status.throttleValueRaw, status.throttleValuePerMille,
                status.throttleValuePerMilleSmooth, status.throttleValueDac,
                MOTOR_SELECTED);
        ...
        sprintf(buffer,
                "%d - Starboard (default) Throttle (raw/per mille/smooth): "
                "[%d/%d/%d], out: [%d] to: [%dkw] motor.\n\r",
                counter, status.throttleValueRaw, status.throttleValuePerMille,
                status.throttleValuePerMilleSmooth, status.throttleValueDac,
                MOTOR_SELECTED);
  ...
  case 3:
    ...
    if (status.throttleStopSwitch == 1) {
      sprintf(buffer,
              "%d - Emergency Stop! Max regen set! Raw/per mille/Smooth: "
              "[%d/%d/%d], out: [%d] to: [%dkw] motor.\n\r",
              counter, status.regenValueRaw, status.regenValuePerMille,
              status.regenValuePerMilleSmooth, status.regenValueDac,
              MOTOR_SELECTED);
    } else {
      sprintf(buffer,
              "%d - Regen (Raw/smooth): [%d/%d/%d], out: [%d] to: [%dkw] "
              "motor.\n\r",
              counter, status.regenValueRaw, status.regenValuePerMille,
              status.regenValuePerMilleSmooth, status.regenValueDac,
              MOTOR_SELECTED);
  ...
  case 4:
    ...
    if (status.throttleStopSwitch == 1) {
      sprintf(buffer,
              "%d - Emergency Stop! Throttle in Neutral, Regen Max!.\n\r",
              counter);
    } else {
      ...
      if (status.throttleValuePerMille == 500) {
        ...
        if (status.throttlePortSelect == 1) {
          // Port throttle active
          sprintf(buffer,
                  "%d - Reverse: [%d%%], Regen: [%d%%] (Port Throttle to: "
                  "[%dkw] motor).\n\r",
                  counter, throttlePosition, regenPosition, MOTOR_SELECTED);
        } else if (status.throttleStarboardSelect == 1) {
          // Starboard throttle active
          sprintf(buffer,
                  "%d - Reverse: [%d%%], Regen: [%d%%] (Starboard Throttle to: "
                  "[%dkw] motor).\n\r",
                  counter, throttlePosition, regenPosition, MOTOR_SELECTED);
        } else {
          // Remote (eventually, Starboard now) throttle active
          sprintf(buffer,
                  "%d - Reverse: [%d%%], Regen: [%d%%] (Remote Throttle to: "
                  "[%dkw] motor).\n\r",
                  counter, throttlePosition, regenPosition, MOTOR_SELECTED);
        }
      }
